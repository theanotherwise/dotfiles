...
- [`.dotfiles_functions`](./.dotfiles_functions) contains reusable shell helper functions, including helpers that back aliases and the multi-repository `git-scan` status table. Repository-owned shell function names must use the `sc_helper_` prefix, with public command names exposed through aliases in [`.dotfiles_aliases`](./.dotfiles_aliases).

...
After adding the binary config, update the shell integration surface as needed: add the tool to the [`.bash_profile`](./.bash_profile) PATH loop, add or update `sc-versions`/`dotversions` metadata in [`.bash_functions`](./.bash_functions), keep category names in English, write a useful short description and example, and add aliases only in [`.bash_aliases`](./.bash_aliases). If the tool needs shell initialization, implement an idempotent `sc_helper_<tool>_init` function in [`.bash_functions`](./.bash_functions) and call it from [`.bash_init`](./.bash_init); do not place raw init blocks in [`.bashrc`](./.bashrc) or [`.bash_profile`](./.bash_profile).

Completion support must be checked for every new CLI. Inspect `--help`, `completion`, `completion bash`, `generate-completion`, `gen-completions`, `shell-completion`, or upstream docs to determine whether Bash completion exists; if it does, load it through [`.bash_completion`](./.bash_completion) using the existing cache helper, and wire aliases to the upstream completion function when aliases are added. If a tool has no built-in completion, state that in the change summary instead of silently skipping it.

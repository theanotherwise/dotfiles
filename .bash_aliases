...
alias decodecert='python3 -c "from cryptography import x509; from cryptography.hazmat.backends import default_backend; import sys; cert_data = sys.stdin.buffer.read() if len(sys.argv) == 1 else open(sys.argv[1], \"rb\").read(); cert = x509.load_pem_x509_certificate(cert_data, default_backend()); print(cert.public_bytes(Encoding.PEM).decode())"'
